function Update_Slice_Lines(PPA)
  ...
  if ~PPA.isVolData &&~isempty(PPA.HorLine)
    delete(PPA.HorLine.ctrHandle);
    delete(PPA.HorLine.upHandle);
    delete(PPA.HorLine.lowHandle);
    delete(PPA.VertLine.ctrHandle);
    delete(PPA.VertLine.upHandle);
    delete(PPA.VertLine.lowHandle);
    PPA.GUI.SliceWidthEditField.Enable = false;

  elseif PPA.isVolData% we have volume data, so show slice lines
    PPA.GUI.SliceWidthEditField.Enable = true;
    sliceWidth = PPA.GUI.SliceWidthEditField.Value; % convert from micron to mm
    ctrPos = PPA.lineCtr;

    % find plot positions for lines and plot them
    xCtr = ctrPos(1);
    yCtr = ctrPos(2);
    uiAx = PPA.GUI.FiltDisp;
